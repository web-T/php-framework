<script type='text/javascript'>
    //<[CDATA[
    var _debug_cats = {%$CATS_JS|@json_encode%};
    ...
    var _debug_data = "<div id='_webt_debug'><div class='_dbg_menu'>" +
            "{%foreach from=$CATS item=cat key=k%}<span id='_debug_btn[{%$k%}]' class='{%$cat.error|escape%}' style='{%if $cat.img%}background-image: url({%$cat.img%}){%/if%}' onclick='_debug_op_close(\"{%$k%}\")'>{%$k|mb_strtoupper%}: {%$cat.value%}{%if $cat.value_detailed%} ({%$cat.value_detailed%}){%/if%}</span>{%/foreach%}" +
            "</div>{%foreach from=$CATS_CONTAINERS item=cat key=k%}<div id='_webt_debug_container[{%$k%}]' class='_debug_container' style='display: none'>{%$cat%}</div>{%/foreach%}<div id='_webt_debug_resizer' on1mouseup='_deb_splitter_release(event)' on1mousedown='_deb_splitter_prepare(event)' >&nbsp;</div></div>";
