...
*/
require_once substr(__FILE__, 0, strrpos(__FILE__, '/')).'/Encode.php';
require_once substr(__FILE__, 0, strrpos(__FILE__, '/')).'/Exception.php';
...
class File_Bittorrent2_Decode
{
    ...
    * @param string
    * @return mixed
    * @throws Exception if decoded data contains trailing garbage
    */
    function decode($str)
    {
        $this->source = $str;
        ...
        if ($this->position < $this->source_length) {
			throw new Exception('Trailing garbage in file.');
        }
        return $result;
    }
    ...
    * @throws Exception if no file given or bencoded data is corrupt
    */
    function decodeFile($file)
    {
        // Check file
        if (!is_file($file)) {
            throw new Exception('Given filename \'' . $file . '\' is not a valid file.');
        }
        ...
        if (!is_array($this->decoded)) {
			throw new Exception('Corrupted bencoded data. Failed to decode data from file \'$file\'.', Exception::decode);
        }
    ...
    */
    protected function decode_dict()
    {
        ...
        while ($char = $this->getChar()) {
            ...
            if (!ctype_digit($char)) {
				throw new Exception('Invalid dictionary key.', Exception::decode);
            }
            $key = $this->decode_string();
            if (isset($return[$key])) {
                throw new Exception('Duplicate dictionary key.', Exception::decode);
            }
            if ($key < $lastkey) {
                throw new Exception('Missorted dictionary key.', Exception::decode);
            }
            $val = $this->bdecode();
            if ($val === false) {
                throw new Exception('Invalid value.', Exception::decode);
            }
            $return[$key] = $val;
            $lastkey = $key;
        }
        if (!$ended) {
            throw new Exception('Unterminated dictionary.', Exception::decode);
        }
        $this->position++;
        return $return;
    }
    ...
    */
    protected function decode_string()
    {
        ...
        substr($this->source, $this->position + 1, 1) != ':') {
			throw new Exception('Leading zero in string length.');
        }
    ...
    */
    protected function decode_int()
    {
        ...
        $p = $this->position;
        if ($p === $pos_e) {
            throw new Exception('Empty integer.', Exception::decode);
        }
        if (substr($this->source, $this->position, 1) == '-') $p++;
        if (substr($this->source, $p, 1) == '0' and
        ($p != $this->position or $pos_e > $p+1)) {
            throw new Exception('Leading zero in integer.', Exception::decode);
        }
        for ($i = $p; $i < $pos_e-1; $i++) {
            if (!ctype_digit(substr($this->source, $i, 1))) {
                throw new Exception('Non-digit characters in integer.', Exception::decode);
            }
        }
    ...
    */
    protected function decode_list()
    {
        ...
        $p1 = $p2 = 0;
        if ($char === false) {
            throw new Exception('Unterminated list.', Exception::decode);
        }
    ...
    */
    function getStats()
    {
        ...
        if (!ini_get('allow_url_fopen')) {
            throw new Exception('\'allow_url_fopen\' must be enabled.', Exception::source);
            return false;
        }
        ...
        try {
			...
			throw new Exception('Invalid scrape data: \'' . $scrape_data . '\'', Exception::decode);
		}
